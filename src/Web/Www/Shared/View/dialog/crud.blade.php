<form id="dialog-form" {!! $action !!} autocomplete="off" {{ $attributes->merge(['class' => 'py-4 px-6']) }}>
    <x-bear::form.csrf/>
    {!!$slot!!}
</form>
<div class="border-t flex gap-2 py-3  px-4 flex-row-reverse">
    ...
    <x-bear::button-dark form="dialog-form" type="submit" hx-dialog-close>{{$submitText}}</x-bear::button-dark>
</div>
