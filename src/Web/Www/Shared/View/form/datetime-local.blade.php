...
<div {{ $attributes->whereStartsWith('class') }}>
    @if($attributes->has('required'))
        <div class="flex justify-between">
            <label for="{{ $id }}" class="block text-sm font-bold mr-4 text-gray-700">{{ $label }}</label>
    ...
    @endif
    <div class="mt-0.5">
        <input type="datetime-local" name="{{ $id }}" id="{{ $id }}" value="{{ $slot }}" aria-describedby="{{ $label }}" {{ $attributes->whereDoesntStartWith('class') }}
