<x-bear::dialog.crud title="Update" endpoint='{!! "/bear/access/token/app/$access_token->id"!!}' class="w-[50rem]">
    <div class="grid grid-cols-2 gap-4">
        ...
        <x-bear::form.text id="access_token_purpose" required pattern="[a-z0-9_]+" placeholder="Describe the usage of the token.." class="col-span-2">{{$access_token->access_token_purpose}}</x-bear::form.text>
        <x-bear::form.text id="route_prefix_restriction" required>{{$access_token->route_prefix_restriction}}</x-bear::form.text>
        <x-bear::form.text id="server_hostname_restriction">{{$access_token->server_hostname_restriction}}</x-bear::form.text>
