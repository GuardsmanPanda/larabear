...
class LarabearCleanLogTablesCommand extends Command {
    ...
    public function handle(): void {
        ...
        $res = BearLogConsoleEvent::where(column: 'created_at', operator: '<', value: now()->subDays($days_to_store_database_changes))->delete();
        if ($res > 0) {
        ...
        $res = BearLogResponseError::where(column: 'created_at', operator: '<', value: now()->subDays($days_to_store_database_changes))->delete();
        if ($res > 0) {
